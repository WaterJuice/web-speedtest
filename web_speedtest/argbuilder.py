    ...
    def parse(self, argv: list[str] | None = None) -> Namespace:
        ...
        if command_names:
            ...
            if not has_command:
                ...
                # Check if there are non-flag arguments (potential positional args
                # that belong to the default command)
                has_positional = any(not arg.startswith("-") for arg in reordered_argv)

                if (is_help or is_version) and not has_positional:
                    # User wants help or version with no positional args —
                    # let argparse show the top-level help/version
                    pass
